<script lang="ts">
	...
	const condition = parseInt(page.params.condition ?? '1') as 1 | 2 | 3 | 4 | 5;
	const showDog = condition === 4 || condition === 5;
	const enforced = condition === 5;

	// ── Prolific ───────────────────────────────────────────────────────────────
	const prolificPid = page.url.searchParams.get('PROLIFIC_PID') ?? null;
	const studyId = page.url.searchParams.get('STUDY_ID') ?? null;
	const sessionId = page.url.searchParams.get('SESSION_ID') ?? null;
	...
	const getSessionData = () => {
		...
		return {
			uid, condition, pretext: preScreenReflection, tasks: taskGroups,
			prolific: { pid: prolificPid, studyId, sessionId }
		};
	};
...
{:else}
	...
	<div class="fixed left-3 top-3 flex flex-col gap-1 font-mono text-[10px]">
		<div class="rounded bg-black/70 px-2 py-1 text-white backdrop-blur flex flex-col gap-0.5">
			...
			<div><span class="text-neutral-400">pid=</span>{prolificPid ?? '—'}</div>
		</div>
